...
RUNTIME = Path(__file__).with_name("Invoke-CyberRangeAgentCouncil.py")
...
GROUPS = ("schema", "lifecycle", "stages", "routes", "provider", "authoring", "security", "independent-hold", "bundle", "authoring-cli", "codex-adapter", "claude-adapter", "changed-candidate-lifecycle")
...
def behavioral_evidence(candidate: dict[str, Any], run_id: str = "test-normative", cycle_id: str = "cycle-current", evidence_kind: str = "behavioral_test") -> dict[str, Any]:
    return {
        "schema_version": "acpl.cyberrange.agent-council-behavioral-evidence/v2",
...
def registry(case_id: str) -> dict[str, Any]:
    ...
    value = {"schema_version": "acpl.cyberrange.agent-model-registry/v2", "registry_version": "v2", "reasoning": {"default_effort": "high"}, "provider_states": {"codex": {"council_status": "qualified"}, "claude_code": {"council_status": "fail_closed_until_qualified"}}, "bindings": bindings}
...
def base_case(case_name: str, root: Path) -> dict[str, Any]:
    ...
    if case_name == "candidate_transition_metadata_as_behavioral_evidence":
        ...
        document["evidence_content"] = {"schema_version": "acpl.cyberrange.agent-council-candidate-transition/v2", "candidate_id": candidate_id, "candidate_sha256": candidate_sha256, "cycle_id": cycle_id, "prior_candidate_id": "candidate-prior"}
    ...
    if case_name == "changed_candidate_new_cycle":
        transition_evidence = {"schema_version": "acpl.cyberrange.agent-council-candidate-transition/v2", "candidate_id": candidate_id, "candidate_sha256": candidate_sha256, "cycle_id": cycle_id, "prior_candidate_id": "candidate-prior"}
...
def write_case(case_name: str, parent: Path) -> dict[str, Any]:
    ...
    if any(item["record_type"] == "evidence" and item["payload"].get("path") == "evidence/transition.json" for item in document["events"]):
        transition_evidence = document.pop("transition_evidence_content", {"schema_version": "acpl.cyberrange.agent-council-candidate-transition/v2"})
...
def schema_validator(path: Path) -> dict[str, Any]:
    schema = json.loads(path.read_text(encoding="utf-8"))
    if schema.get("$schema") != "https://json-schema.org/draft/2020-12/schema" or not isinstance(schema.get("$id"), str):
        raise ValueError(f"{path.name} is not identified as a Draft 2020-12 schema")
...
def document_schema_errors(name: str, value: Any) -> list[str]:
    ...
    if name == "bundle":
        ...
        valid = required <= value.keys() and value["schema_version"] == "acpl.cyberrange.agent-council-case/v2" and value["route"] in {"R1", "R2", "R3"} and identity.get("registry_version") == "v2" and {"candidate_sha256", "dependency_lock_sha256", "policy_sha256"} <= candidate.keys()
        return [] if valid else ["bundle required fields are invalid"]
    if name == "registry":
        return [] if value.get("schema_version") == "acpl.cyberrange.agent-model-registry/v2" and value.get("reasoning", {}).get("default_effort") == "high" and value.get("bindings") else ["registry required fields are invalid"]
    if name == "capability":
...
def test_schema() -> list[tuple[str, bool, str]]:
    ...
    try:
        for name in SCHEMAS: validators[name] = schema_validator(PACKAGE / "standards" / name); rows.append((name, True, "valid Draft 2020-12 schema with an identifier"))
...
def runtime_red_cases(group: str) -> list[tuple[str, bool, str]]:
    ...
    if group == "independent-hold": names = ["required_gate_without_test", "closure_self_declared_gate", "prior_candidate_evidence_reuse"]
    if group == "authoring": names.append("changed_candidate_new_cycle")
    rows: list[tuple[str, bool, str]] = []; api, runtime_error = load_runtime(); validators: dict[str, dict[str, Any]] = {name: schema_validator(PACKAGE / "standards" / name) for name in SCHEMAS}
...
def cli_detail(*args: str) -> tuple[int, dict[str, Any], str]:
    completed = subprocess.run([sys.executable, str(RUNTIME), *args], capture_output=True, text=True, check=False)
    try:
...
def test_public_cli_boundaries() -> list[tuple[str, bool, str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-council-v2-cli-boundaries-") as temporary:
        ...
        concurrent_command = [sys.executable, str(RUNTIME), "resume", "--case-root", str(concurrent_root), "--interruption-id", concurrent_interruption.get("interruption_id", "missing")]
        ...
        rows.append(("manifest-status-promotion-preserves-behavioral-candidate-digest", manifest_code == 0 and "scripts/manifest.yaml" not in artifact_pins and stable_before == stable_after and status_only_promotion["status"] == "promoted", "status-only manifest promotion is outside the behavioral candidate artifact digest"))
...
def test_public_json_depth_guard() -> list[tuple[str, bool, str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-council-v2-json-depth-") as temporary:
        ...
        for label, args in argument_cases:
            before = filesystem_inventory(case_root) if init_code == 0 else {}
...
def test_public_complete_envelope_depth_guard() -> list[tuple[str, bool, str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-council-v2-envelope-depth-") as temporary:
        ...
        for record_type, payload in append_cases:
            ...
            rows.append((f"append-{record_type}-61-array-metadata-refuses-before-any-write", init_code == 0 and code != 0 and structural_refusal(response, "SCHEMA_VALIDATION_FAILED") and no_traceback(response, stderr) and after == before and after_events == before_events and after_state == before_state, "metadata that becomes over-depth only in the complete immutable envelope refuses without lock, event, evidence, state, or filesystem mutation"))
...
def test_authoring(group: str) -> list[tuple[str, bool, str]]:
    ...
    with tempfile.TemporaryDirectory(prefix="agent-council-v2-authoring-") as temporary:
        ...
        concurrent_commands = [[sys.executable, str(RUNTIME), "evidence-add", "--case-root", str(case_root), "--evidence-id", evidence_id, "--evidence-kind", "behavioral_test", "--content-json", json.dumps(behavioral_evidence(initialized_candidate, f"test-{evidence_id}", initial_cycle))] for evidence_id in ("evidence-concurrent-a", "evidence-concurrent-b")]
...
def run(group: str) -> dict[str, Any]:
    ...
    elif group in {"bundle", "authoring-cli", "codex-adapter", "claude-adapter", "changed-candidate-lifecycle"}: rows = test_authoring(group)
