...
or a Lab. It validates receipt shape and local record linkage only. It cannot
...
_BEHAVIORAL_EVIDENCE_SCHEMA = "acpl.cyberrange.agent-council-behavioral-evidence/v2"
_MAX_JSON_DEPTH = 64
...
def _security_problem(value: Any) -> str | None:
    ...
    while pending:
        ...
        elif isinstance(current, str) and any(ord(char) < 32 or ord(char) == 127 for char in current):
            return "CONTROL_CHARACTER_REFUSED"
    return None
...
def _document_valid(case: Any) -> bool:
    ...
    if not isinstance(case, dict) or not required <= case.keys() or case.get("schema_version") != "acpl.cyberrange.agent-council-case/v2":
        return False
...
def _candidate_transition_metadata_valid(content: Any, payload: dict[str, Any], event: dict[str, Any]) -> bool:
    if not isinstance(content, dict) or payload.get("kind") != "candidate_transition_metadata":
        return False
    if content.get("schema_version") != "acpl.cyberrange.agent-council-candidate-transition/v2":
        return False
...
_PINNED_ARTIFACTS = (
    "scripts/core/agent-council/0.2.0/Invoke-CyberRangeAgentCouncil.py",
    "scripts/core/agent-council/0.2.0/Bootstrap-CyberRangeAgentCouncil.py",
    "scripts/core/agent-council/0.2.0/Test-CyberRangeAgentCouncilContracts.py",
...
def _authoring_state_problem(state: Any) -> str | None:
    ...
    if issue:
        return issue
    required = {"authoring_schema_version", "case_id", "case_root", "route", "activation_class", "case_kind", "cycle_id", "policy_bundle", "engineering_candidate", "registry", "events", "capabilities", "dispatch_plans", "dispatch_receipts"}
    if not isinstance(state, dict) or not required <= state.keys() or state.get("authoring_schema_version") != "acpl.cyberrange.agent-council-authoring/v2" or not isinstance(state.get("events"), list) or not isinstance(state.get("capabilities"), dict) or not isinstance(state.get("dispatch_plans"), dict) or not isinstance(state.get("dispatch_receipts"), dict):
        return "SCHEMA_VALIDATION_FAILED"
...
def _authoring_lock(root: Path):
    ...
    # Lock the already-persisted state file rather than creating a sentinel.
    # A refused command must not leave a newly-created lock artifact behind.
    lock_path = root / "case.json"
...
def _read_local_case(root: Path) -> dict[str, Any]:
    try:
        state = _decode_json((root / "case.json").read_text(encoding="utf-8"))
    ...
    if state.get("authoring_schema_version") != "acpl.cyberrange.agent-council-authoring/v2":
...
def init_case(root: Path, case_id: str, route: str, candidate: dict[str, Any] | None = None, activation_class: str | None = None, case_kind: str = "operational_change") -> dict[str, Any]:
    ...
    state = {"authoring_schema_version": "acpl.cyberrange.agent-council-authoring/v2", "case_id": case_id, "case_root": str(root.resolve()), "route": route, "activation_class": activation_class, "case_kind": case_kind, "cycle_id": "cycle-000001", "policy_bundle": {"policy_bundle_id": "policy-v2", "policy_sha256": policy_sha, "artifact_pins": pins}, "engineering_candidate": dict(candidate_hashes, candidate_id=f"candidate-{candidate_sha[:24]}", candidate_sha256=candidate_sha), "registry": registry, "events": [], "capabilities": {}, "dispatch_plans": {}, "dispatch_receipts": {}}
...
def _invalidate_dispatch_plans(state: dict[str, Any]) -> None:
    for plan in state.get("dispatch_plans", {}).values():
        if not plan.get("consumed"):
            plan["invalidated"] = True


def dispatch_plan(root: Path, provider: str, role: str, context_id: str, effort: str = "high", decision_kind: str | None = None, qualification_run_id: str | None = None) -> dict[str, Any]:
    if any(_public_text_problem(value) for value in (provider, role, context_id, effort)) or (decision_kind is not None and _public_text_problem(decision_kind)) or (qualification_run_id is not None and _public_text_problem(qualification_run_id)):
        raise AuthoringError("SCHEMA_VALIDATION_FAILED")
    ...
    request = {"plan_nonce": nonce, "identity": _dispatch_plan_identity(state), "provider": provider, "role": role, "model_id": binding["model_id"], "binding_id": binding["binding_id"], "binding_sha256": binding["binding_sha256"], "effort": effort, "context_id": context_id, "decision_kind": decision_kind, "qualification_run_id": qualification_run_id}
    if role in {"technical_tactical_intelligence", "worker_intelligence"}:
...
def _dispatch_capture_plan_problem(state: dict[str, Any], plan: Any, nonce: Any, root: Path) -> str | None:
    ...
    if not isinstance(role, str) or role not in _ROLES:
        return "SCHEMA_VALIDATION_FAILED"
    request_keys = {"plan_nonce", "identity", "provider", "role", "model_id", "binding_id", "binding_sha256", "effort", "context_id", "decision_kind", "qualification_run_id"}
    if role in {"technical_tactical_intelligence", "worker_intelligence"}:
...
def _authoring_document(state: dict[str, Any], case_root: Path, events: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    if not capability:
        raise AuthoringError("CAPABILITY_REQUIRED")
    return {
        "schema_version": "acpl.cyberrange.agent-council-case/v2",
...
def cycle_start(root: Path, candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    transition_content = {"schema_version": "acpl.cyberrange.agent-council-candidate-transition/v2", "candidate_id": proposed["engineering_candidate"]["candidate_id"], "candidate_sha256": candidate_sha, "cycle_id": proposed["cycle_id"], "prior_candidate_id": prior}
    try:
...
def review_record(root: Path, review_id: str, reviewer_id: str, implementation_id: str, evidence_id: str) -> dict[str, Any]:
    ...
    state = _read_local_case(root); current = [event for event in state["events"] if event["cycle_id"] == state["cycle_id"] and event["candidate_id"] == state["engineering_candidate"]["candidate_id"] and event["record_type"] == "dispatch"]
...
def gate_record(root: Path, gate_id: str, evidence_id: str, cause: str | None = None) -> dict[str, Any]:
    ...
    state = _read_local_case(root); payload = {"gate_id": gate_id, "status": "passed", "evidence_id": evidence_id, "system_generated": True}
    cycle_events = [event for event in state["events"] if event["cycle_id"] == state["cycle_id"] and event["candidate_id"] == state["engineering_candidate"]["candidate_id"]]
    if evidence_id not in _behavioral_evidence_ids(root, cycle_events, state["engineering_candidate"], state["cycle_id"]):
        raise AuthoringError("REQUIRED_GATE_EVIDENCE_MISSING")
    if cause:
...
def interrupt_case(root: Path, processes: list[Any]) -> dict[str, Any]:
    ...
    state = _read_local_case(root); event = _append_local_event(root, state, "interruption", {"status": "interrupted", "owned_processes": processes}, f"interruption-{len(state['events']) + 1:06d}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    init.add_argument("--case-root", required=True, type=Path); init.add_argument("--case-id", required=True); init.add_argument("--route", required=True); init.add_argument("--activation-class"); init.add_argument("--case-kind", default="operational_change"); init.add_argument("--candidate-json")
    bundle = commands.add_parser("build-bundle", help="build an executing bundle from pinned artifacts")
    bundle.add_argument("--case-root", required=True, type=Path)
    append = commands.add_parser("append", help="append a non-protected local event")
    append.add_argument("--case-root", required=True, type=Path); append.add_argument("--record-type", required=True); append.add_argument("--payload-json", required=True)
    evidence = commands.add_parser("evidence-add", help="add immutable candidate-bound behavioral evidence")
    evidence.add_argument("--case-root", required=True, type=Path); evidence.add_argument("--evidence-id", required=True); evidence.add_argument("--evidence-kind", required=True, choices=sorted(_BEHAVIORAL_EVIDENCE_KINDS)); evidence.add_argument("--content-json", required=True)
    capability = commands.add_parser("capability-capture", help="record a current adapter capability observation")
    capability.add_argument("--case-root", required=True, type=Path); capability.add_argument("--provider", required=True); capability.add_argument("--model-id", required=True); capability.add_argument("--efforts-json", required=True); capability.add_argument("--context-id", required=True); capability.add_argument("--observed-at")
    plan = commands.add_parser("dispatch-plan", help="issue one single-use adapter dispatch plan")
    plan.add_argument("--case-root", required=True, type=Path); plan.add_argument("--provider", required=True); plan.add_argument("--role", required=True); plan.add_argument("--context-id", required=True); plan.add_argument("--effort", default="high"); plan.add_argument("--decision-kind"); plan.add_argument("--qualification-run-id")
    ...
    close.add_argument("--case-root", required=True, type=Path)
    cycle = commands.add_parser("cycle-start", help="start a changed engineering candidate cycle")
    cycle.add_argument("--case-root", required=True, type=Path); cycle.add_argument("--candidate-json", required=True)
    request = commands.add_parser("packet-request", help="record an accepted bounded packet")
    request.add_argument("--case-root", required=True, type=Path); request.add_argument("--packet-id", required=True); request.add_argument("--role", required=True)
    packet_result_parser = commands.add_parser("packet-result", help="record a packet's actual checks")
    packet_result_parser.add_argument("--case-root", required=True, type=Path); packet_result_parser.add_argument("--packet-id", required=True); packet_result_parser.add_argument("--status", required=True); packet_result_parser.add_argument("--checks-json", required=True)
    test_record_parser = commands.add_parser("test-record", help="record candidate-bound passing test evidence")
    test_record_parser.add_argument("--case-root", required=True, type=Path); test_record_parser.add_argument("--run-id", required=True); test_record_parser.add_argument("--evidence-id", required=True); test_record_parser.add_argument("--status", default="passed"); test_record_parser.add_argument("--test-kind", default="normative")
    review = commands.add_parser("review-record", help="record independent review")
    review.add_argument("--case-root", required=True, type=Path); review.add_argument("--review-id", required=True); review.add_argument("--reviewer-id", required=True); review.add_argument("--implementation-id", required=True); review.add_argument("--evidence-id", required=True)
    gate = commands.add_parser("gate", help="record a system-generated passing gate")
    gate.add_argument("--case-root", required=True, type=Path); gate.add_argument("--gate-id", required=True); gate.add_argument("--evidence-id", required=True); gate.add_argument("--cause")
    interrupt = commands.add_parser("interrupt", help="record an interruption and owned process reconciliation")
    interrupt.add_argument("--case-root", required=True, type=Path); interrupt.add_argument("--processes-json", required=True)
    resume = commands.add_parser("resume", help="exactly resume a reconciled interruption in a new cycle")
    resume.add_argument("--case-root", required=True, type=Path); resume.add_argument("--interruption-id", required=True)
    ...
    try:
        ...
        with lock:
            if args.command == "init-case": result = init_case(args.case_root, args.case_id, args.route, candidate=parsed.get("candidate"), activation_class=args.activation_class, case_kind=args.case_kind)
            ...
            elif args.command == "dispatch-plan": result = dispatch_plan(args.case_root, args.provider, args.role, args.context_id, args.effort, args.decision_kind, args.qualification_run_id)
    ...
    if not isinstance(document, dict):
        print(json.dumps(_refuse("SCHEMA_VALIDATION_FAILED"), sort_keys=True))
        return 1
    if document.get("authoring_schema_version") == "acpl.cyberrange.agent-council-authoring/v2":
        try:
